...
function Sidebar(props) {
    ...
    const smartCount = (scope) => {
        return activeCards.filter(card => {
            ...
            if (scope === "waiting")
                return (!!bic && bic.id !== data.settings.myActorId) || card.statusId === "status-waiting";
            if (scope === "no-date")
                return !date;
            return true;
        }).length;
...
class RunningTaskApp extends Component {
    constructor() {
        ...
        this.restoreCard = (cardId) => this.updateData(data => { const card = data.cards.find(c => c.id === cardId); if (card) {
            ...
            card.statusId = valid ? String(card.lastActiveStatusId) : "status-todo";
        ...
        this.deleteCard = (cardId) => {
            if (!window.confirm("Permanently delete this task and its checklist? This cannot be undone except from a backup."))
                return;
        ...
        this.setSetting = (key, value) => this.updateData(data => { data.settings[key] = value; });
        ...
        this.restoreBackup = async (id) => {
            if (!window.confirm("Restore this backup? The current workspace will be backed up first, then replaced."))
                return;
            this.setState({ backupBusy: true });
            try {
                if (!await this.flushPendingSave(false))
                ...
                this.setState({ data, drawerCardId: null, expanded: new Set(), undoSnapshot: previous, undoLabel: "Restore backup", saveState: "saved" });
                await this.refreshBackups();
        ...
        this.confirmImport = async () => {
            ...
            try {
                ...
                this.suppressNextAutosave = true;
                this.setState({ data: imported, importCandidate: null, drawerCardId: null, expanded: new Set(), selectedTopicId: null, scope: "", route: "dashboard", undoSnapshot: previous, undoLabel: "Import workspace", saveState: "saved" });
                await this.refreshBackups();
    ...
    filteredCards() {
        ...
        return data.cards.filter(card => !isDoneCard(data, card)).filter(card => {
            ...
            if (this.state.scope === "my-ball" && (bic === null || bic === void 0 ? void 0 : bic.id) !== data.settings.myActorId)
                return false;
            if (this.state.scope === "waiting" && !((bic && bic.id !== data.settings.myActorId) || card.statusId === "status-waiting"))
                return false;
    ...
    render() {
        ...
        const drawerCard = this.state.drawerCardId ? data.cards.find(c => c.id === this.state.drawerCardId && !c.isArchived) : undefined;
