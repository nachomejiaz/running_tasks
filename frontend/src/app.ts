...
interface Card {
  ...
  fallbackBicId: string | null;
  tags: string[];
  rank: number;
  isArchived: boolean;
  archiveReason: string | null;
  lastActiveStatusId: string | null;
  completedAt: string | null;
  createdAt: string;
  updatedAt: string;
}
...
interface Filters {
  statusId: string;
  bicId: string;
  due: "all" | "overdue" | "today" | "7days" | "30days" | "nodate";
  priority: string;
}
type RouteName = "dashboard" | "board" | "list" | "calendar" | "flow" | "archive" | "settings";
type SmartScope = "" | "today" | "overdue" | "my-ball" | "waiting" | "no-date";
interface BackupInfo { id: string; name: string; path: string; createdAt: string; sizeBytes: number; }
interface StorageInfo { dataPath: string; backupPath: string; mode: string; }
interface ImportSummary { topics: number; subtopics: number; cards: number; checklistItems: number; archivedCards: number; actors: number; }
interface ImportCandidate { fileName: string; data: AppData; summary: ImportSummary; warnings: string[]; }

function uid(prefix: string): string {
  const cryptoAny = (window as any).crypto;
  if (cryptoAny && typeof cryptoAny.randomUUID === "function") return `${prefix}-${cryptoAny.randomUUID()}`;
...
function currentMonthKey(): string { return monthKey(new Date()); }
function parseMonthKey(value: string): Date {
  const parsed = parseDateOnly(value);
  if (!parsed) return new Date(new Date().getFullYear(), new Date().getMonth(), 1, 12, 0, 0, 0);
...
function calendarDayLabel(day: CalendarDay): string {
  if (day.date.getDate() === 1) return day.date.toLocaleDateString(undefined, { month: "short", day: "numeric" });
...
function dayOffset(value: string | null): number | null {
  const parsed = parseDateOnly(value);
...
function sortRank<T extends { rank: number }>(items: T[]): T[] { return items.slice().sort((a, b) => a.rank - b.rank); }
function stop(event: any): void { event.stopPropagation(); }
function isTypingTarget(target: any): boolean {
  const tag = String(target && target.tagName || "").toLowerCase();
  return tag === "input" || tag === "textarea" || tag === "select" || !!(target && target.isContentEditable);
}
function statusById(data: AppData, id: string): Status | undefined { return data.statuses.find(s => s.id === id); }
function topicById(data: AppData, id: string | null): Topic | undefined { return data.topics.find(t => t.id === id); }
function subtopicById(data: AppData, id: string | null): Subtopic | undefined { return data.subtopics.find(t => t.id === id); }
function actorById(data: AppData, id: string | null): Actor | undefined { return data.actors.find(a => a.id === id); }
function typeById(data: AppData, id: string | null): CardType | undefined { return data.cardTypes.find(t => t.id === id); }
function checklistFor(data: AppData, cardId: string): ChecklistItem[] { return data.checklistItems.filter(i => i.cardId === cardId).sort((a, b) => a.rank - b.rank); }
function openChecklistFor(data: AppData, cardId: string): ChecklistItem[] { return checklistFor(data, cardId).filter(i => !i.completed); }
function nextItemFor(data: AppData, card: Card): ChecklistItem | null {
...
function nextDateFor(data: AppData, card: Card): string | null {
  const item = nextItemFor(data, card);
  return item && item.dueDate ? item.dueDate : card.targetDate;
}
function nextBicFor(data: AppData, card: Card): Actor | undefined {
  const item = nextItemFor(data, card);
  return actorById(data, item && item.bicId ? item.bicId : card.fallbackBicId);
}
function progressFor(data: AppData, card: Card): { done: number; total: number; percent: number } {
...
function dueClass(value: string | null): string {
  const offset = dayOffset(value);
...
function normalizeText(value: any): string { return String(value || "").toLowerCase().trim(); }
function csvEscape(value: any): string {
  const text = String(value === null || value === undefined ? "" : value);
  return /[",\r\n]/.test(text) ? `"${text.replace(/"/g, '""')}"` : text;
}
function downloadTextFile(name: string, content: string, type: string): string {
...
function workspaceCsv(data: AppData): string {
  const headers = [
  ...
  cards.forEach(card => {
    const topic = topicById(data, card.topicId)?.name || "";
    const subtopic = subtopicById(data, card.subtopicId)?.name || "";
    const cardType = typeById(data, card.cardTypeId)?.name || "";
    const status = statusById(data, card.statusId)?.name || "";
    const currentBic = nextBicFor(data, card)?.name || "";
    const items = checklistFor(data, card.id);
    ...
    if (!items.length) rows.push([...cardColumns, "", "", "", "", "", ""]);
    items.forEach(item => rows.push([...cardColumns, item.title, item.parentId ? (byId.get(item.parentId)?.title || "") : "",
      item.dueDate || "", actorById(data, item.bicId)?.name || "", item.completed ? "Yes" : "No", item.notes]));
  });
  return `${rows.map(row => row.map(csvEscape).join(",")).join("\r\n")}\r\n`;
}
function markdownText(value: string): string { return String(value || "").replace(/[\r\n]+/g, " ").trim(); }
function workspaceMarkdown(data: AppData): string {
  const lines: string[] = [`# Running_Task workspace export`, "", `Exported: ${new Date().toLocaleString()}`, `Version: ${APP_VERSION}`, ""];
  ...
  topics.forEach(topic => {
    ...
    if (!topicCards.length) { lines.push("_No tasks._", ""); return; }
    topicCards.forEach(card => {
      const status = statusById(data, card.statusId)?.name || "Unknown";
      ...
      const subtopic = subtopicById(data, card.subtopicId)?.name;
      if (subtopic) lines.push(`- Subtopic: ${markdownText(subtopic)}`);
      const cardType = typeById(data, card.cardTypeId)?.name;
      if (cardType) lines.push(`- Card type: ${markdownText(cardType)}`);
      lines.push(`- Priority: ${card.priority}`);
      lines.push(`- Next date: ${nextDateFor(data, card) || "No date"}`);
      lines.push(`- Ball in Court: ${nextBicFor(data, card)?.name || "Unassigned"}`);
      if (card.description) lines.push("", markdownText(card.description));
      const items = checklistFor(data, card.id);
      if (items.length) {
        ...
        items.forEach(item => {
          ...
          const details = [item.dueDate || "No date", actorById(data, item.bicId)?.name || "No BIC"].join(" | ");
          lines.push(`${indent}- [${item.completed ? "x" : " "}] ${markdownText(item.title)} (${details})`);
        });
      }
      if (card.notes) lines.push("", `Notes: ${markdownText(card.notes)}`);
      lines.push("");
    });
  });
  return `${lines.join("\n").trim()}\n`;
}
function workspaceSummary(data: AppData): ImportSummary {
  return {
...
function validateWorkspaceData(data: AppData): string[] {
  const errors: string[] = [];
  const uniqueIds = (label: string, items: Array<{ id: string }>) => {
    const ids = new Set<string>();
    items.forEach(item => {
      if (!item.id || typeof item.id !== "string") errors.push(`${label} contains a record without an ID.`);
      else if (ids.has(item.id)) errors.push(`${label} contains duplicate ID ${item.id}.`);
      else ids.add(item.id);
    });
  };
  uniqueIds("Topics", data.topics); uniqueIds("Subtopics", data.subtopics); uniqueIds("Card types", data.cardTypes);
  uniqueIds("Statuses", data.statuses); uniqueIds("BIC actors", data.actors); uniqueIds("Cards", data.cards);
  uniqueIds("Checklist", data.checklistItems); uniqueIds("Saved views", data.savedViews);
  if (!data.topics.length) errors.push("At least one Topic is required.");
  if (!data.statuses.length) errors.push("At least one Status is required.");
  if (!data.statuses.some(status => status.terminal)) errors.push("A terminal Done status is required.");
  ...
  data.subtopics.forEach(item => { if (!topicIds.has(item.topicId)) errors.push(`Subtopic ${item.name || item.id} references a missing Topic.`); });
  data.cards.forEach(card => {
    if (!card.title || !card.title.trim()) errors.push(`Card ${card.id} has no title.`);
    if (!topicIds.has(card.topicId)) errors.push(`Card ${card.title || card.id} references a missing Topic.`);
    if (!statusIds.has(card.statusId)) errors.push(`Card ${card.title || card.id} references a missing Status.`);
    if (card.subtopicId) {
      const subtopic = subtopicByKey.get(card.subtopicId);
      if (!subtopic) errors.push(`Card ${card.title || card.id} references a missing Subtopic.`);
      else if (subtopic.topicId !== card.topicId) errors.push(`Card ${card.title || card.id} has a Subtopic from another Topic.`);
    }
    if (card.cardTypeId && !typeIds.has(card.cardTypeId)) errors.push(`Card ${card.title || card.id} references a missing Card Type.`);
    if (card.fallbackBicId && !actorIds.has(card.fallbackBicId)) errors.push(`Card ${card.title || card.id} references a missing BIC actor.`);
  });
  const itemById = new Map(data.checklistItems.map(item => [item.id, item]));
  data.checklistItems.forEach(item => {
    if (!cardIds.has(item.cardId)) errors.push(`Checklist item ${item.title || item.id} references a missing Card.`);
    if (item.bicId && !actorIds.has(item.bicId)) errors.push(`Checklist item ${item.title || item.id} references a missing BIC actor.`);
    if (item.parentId) {
      const parent = itemById.get(item.parentId);
      if (!parent) errors.push(`Checklist item ${item.title || item.id} references a missing parent.`);
      else if (parent.cardId !== item.cardId) errors.push(`Checklist item ${item.title || item.id} has a parent from another Card.`);
      const visited = new Set<string>();
      let cursor: ChecklistItem | undefined = item;
      while (cursor?.parentId) {
        if (visited.has(cursor.id)) { errors.push(`Checklist item ${item.title || item.id} is part of a parent cycle.`); break; }
        visited.add(cursor.id); cursor = itemById.get(cursor.parentId);
      }
    }
  });
  return Array.from(new Set(errors));
}
function prepareImportedWorkspace(raw: any): { data: AppData; summary: ImportSummary; warnings: string[] } {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) throw new Error("The selected file is not a Running_Task workspace object.");
  if (!raw.meta || !Array.isArray(raw.cards) || !Array.isArray(raw.topics)) throw new Error("The selected JSON does not contain Running_Task workspace data.");
  const schemaVersion = Number(raw.meta.schemaVersion || 1);
  if (!Number.isFinite(schemaVersion) || schemaVersion < 1) throw new Error("The workspace schema version is invalid.");
  if (schemaVersion > APP_SCHEMA_VERSION) throw new Error(`This file uses schema ${schemaVersion}, which is newer than this build supports.`);
  ...
  if (errors.length) throw new Error(`Import validation failed: ${errors.slice(0, 6).join(" ")}${errors.length > 6 ? ` Plus ${errors.length - 6} more issue(s).` : ""}`);
  data.meta.schemaVersion = APP_SCHEMA_VERSION;
  data.meta.appVersion = APP_VERSION;
  data.meta.updatedAt = nowIso();
  const warnings: string[] = [];
  if (sourceVersion !== APP_VERSION) warnings.push(`The file was created by Running_Task ${sourceVersion}; it will be normalized for ${APP_VERSION}.`);
  if (!data.cards.length) warnings.push("The imported workspace contains no Cards.");
...
function defaultData(): AppData {
  ...
  const card = (id: string, topicId: string, subtopicId: string | null, typeId: string, statusId: string, reference: string, title: string, description: string, priority: Card["priority"], rank: number): Card => ({
  ...
  const item = (id: string, cardId: string, title: string, dueDate: string | null, bicId: string | null, completed: boolean, rank: number, parentId: string | null = null): ChecklistItem => ({
...
function ensureDataShape(raw: any): AppData {
  ...
  if (!raw || !raw.meta || !Array.isArray(raw.cards)) return fallback;
...
const tauriWindow: any = window as any;
function hasTauri(): boolean { return !!(tauriWindow.__TAURI__ && tauriWindow.__TAURI__.core && tauriWindow.__TAURI__.core.invoke); }
...
interface SaveQueueSnapshot<T> { revision: number; value: T; }
...
class SerializedSaveQueue<T> {
  private pending: SaveQueueSnapshot<T> | null = null;
  private inFlight: Promise<void> | null = null;
  private nextRevision = 0;
  private persistedRevision = 0;

  constructor(private readonly saver: (value: T) => Promise<void>) {}
  ...
  hasWork(): boolean { return !!this.pending || !!this.inFlight; }
  latestRevision(): number { return this.nextRevision; }
  savedRevision(): number { return this.persistedRevision; }
...
class DataProvider {
  ...
  readBrowserBackups(): any[] {
    if (!this.browserStorageAvailable) return this.memoryBackups.slice();
    try {
  ...
  writeBrowserBackups(items: any[]): void {
    this.memoryBackups = deepClone(items);
    if (!this.browserStorageAvailable) return;
    try { localStorage.setItem(BACKUP_KEY, JSON.stringify(items)); }
    catch (_error) { this.browserStorageAvailable = false; }
  }
  ...
  async save(data: AppData): Promise<void> {
    if (this.desktop) { await tauriInvoke("save_app_data", { data }); return; }
    this.memoryData = deepClone(data);
  ...
  ensureBrowserDailyBackup(data: AppData): void {
    const list = this.readBrowserBackups();
    const day = dateOnly(new Date());
    if (!list.some((backup: any) => backup.kind === "automatic" && backup.day === day)) {
      list.unshift({ id: uid("backup"), kind: "automatic", day, name: `Auto_${day}.json`, createdAt: nowIso(), data: deepClone(data) });
    }
    const automatic = list.filter((backup: any) => backup.kind === "automatic").sort((a: any, b: any) => String(b.createdAt).localeCompare(String(a.createdAt)));
    const keep = new Set(automatic.slice(0, Math.max(1, data.settings.backupRetention || 14)).map((backup: any) => backup.id));
    const retained = list.filter((backup: any) => backup.kind !== "automatic" || keep.has(backup.id)).slice(0, 200);
    this.writeBrowserBackups(retained);
  }
  async setAutostart(enabled: boolean): Promise<boolean> {
    if (this.desktop) return await tauriInvoke<boolean>("set_autostart", { enabled });
    return enabled;
  }
  async getAutostart(): Promise<boolean> {
    if (this.desktop) return await tauriInvoke<boolean>("get_autostart");
    return true;
  }
  async listBackups(): Promise<BackupInfo[]> {
    if (this.desktop) return await tauriInvoke<BackupInfo[]>("list_backups");
    ...
    return items.map((b: any) => ({ id: b.id, name: b.name, path: location, createdAt: b.createdAt, sizeBytes: JSON.stringify(b.data).length }));
  }
  async createBackup(data: AppData): Promise<BackupInfo> {
    if (this.desktop) return await tauriInvoke<BackupInfo>("create_backup");
    const list = this.readBrowserBackups();
  ...
  async restoreBackup(id: string): Promise<AppData> {
    if (this.desktop) { await tauriInvoke("restore_backup", { backupId: id }); return await this.load(); }
    const backup = this.readBrowserBackups().find((b: any) => b.id === id);
  ...
  async deleteBackup(id: string): Promise<void> {
    if (this.desktop) { await tauriInvoke("delete_backup", { backupId: id }); return; }
    this.writeBrowserBackups(this.readBrowserBackups().filter((b: any) => b.id !== id));
  }
  async exportJson(data: AppData): Promise<string> {
    if (this.desktop) return await tauriInvoke<string>("export_json");
    return downloadTextFile(`Running_Task_${dateOnly(new Date())}.json`, JSON.stringify(data, null, 2), "application/json");
  }
  async exportCsv(data: AppData): Promise<string> {
    if (this.desktop) return await tauriInvoke<string>("export_csv");
    return downloadTextFile(`Running_Task_${dateOnly(new Date())}.csv`, workspaceCsv(data), "text/csv;charset=utf-8");
  }
  async exportMarkdown(data: AppData): Promise<string> {
    if (this.desktop) return await tauriInvoke<string>("export_markdown");
  ...
  async storageInfo(): Promise<StorageInfo> {
    if (this.desktop) return await tauriInvoke<StorageInfo>("get_storage_info");
    if (!this.browserStorageAvailable) return { dataPath: "Temporary memory for this preview tab", backupPath: "Temporary memory for this preview tab", mode: "Browser preview (storage unavailable)" };
  ...
  async openDataFolder(): Promise<void> {
    if (this.desktop) await tauriInvoke("open_data_folder");
  }
}
...
function ActorPill(props: { actor?: Actor; compact?: boolean }): any {
  if (!props.actor) return html`<span className="bic-pill"><span className="actor-avatar" style=${{ background: "#6b778c" }}>?</span><span className="actor-name">Unassigned</span></span>`;
...
function TypeBadge(props: { type?: CardType }): any {
  if (!props.type) return null;
  return html`<span className="badge" style=${{ color: props.type.color, background: `${props.type.color}18` }}>${props.type.name}</span>`;
}
function PriorityBadge(props: { priority: Card["priority"] }): any {
  if (!props.priority || props.priority === "None" || props.priority === "Normal") return null;
  ...
  return html`<header className="topbar">
    ...
    <div className="topbar-center">
      <div className="search-wrap">
        ...
        <input id="global-search" className="search-input" type="search" value=${props.search} onInput=${(e: any) => props.onSearch(e.target.value)} placeholder="Search tasks, checklists, references, BIC…" aria-label="Search all tasks" />
      ...
      ${showViews ? html`<div className="segmented" aria-label="Visualization mode">
        ${(["board", "list", "calendar", "flow"] as RouteName[]).map(name => html`<button key=${name} className=${`btn ${route === name ? "active" : ""}`} onClick=${() => props.onRoute(name)} title=${name.charAt(0).toUpperCase() + name.slice(1)}><${Icon} name=${name} size=${16}/><span>${name.charAt(0).toUpperCase() + name.slice(1)}</span></button>`)}
      </div>` : null}
    </div>
...
function Sidebar(props: any): any {
  ...
  const activeCards = data.cards.filter((c: Card) => !isDoneCard(data, c));
  const countTopic = (id: string) => activeCards.filter(c => c.topicId === id).length;
  const smartCount = (scope: SmartScope): number => {
  ...
  const nav = (route: RouteName, label: string, icon: string) => html`<button className=${`nav-item ${props.route === route && !props.scope && !props.selectedTopicId ? "active" : ""}`} onClick=${() => props.onNavigate(route, null, "")}><${Icon} name=${icon} size=${18}/><span className="nav-label">${label}</span></button>`;
  const smart = (scope: SmartScope, label: string, icon: string) => html`<button className=${`nav-item ${props.scope === scope ? "active" : ""}`} onClick=${() => props.onNavigate("list", null, scope)}><${Icon} name=${icon} size=${18}/><span className="nav-label">${label}</span><span className="nav-count">${smartCount(scope)}</span></button>`;
...
function ActiveFilterBar(props: any): any {
  const chips: any[] = [];
  const data: AppData = props.data;
  if (props.selectedTopicId) {
    const topic = topicById(data, props.selectedTopicId);
    if (topic) chips.push(html`<span className="filter-chip" key="topic"><span className="topic-dot" style=${{ background: topic.color }}></span>${topic.name}<button onClick=${() => props.onClearTopic()}><${Icon} name="x" size=${13}/></button></span>`);
  }
  if (props.scope) chips.push(html`<span className="filter-chip" key="scope">${String(props.scope).replace(/-/g, " ")}<button onClick=${() => props.onClearScope()}><${Icon} name="x" size=${13}/></button></span>`);
  if (props.filters.statusId) {
    const s = statusById(data, props.filters.statusId); if (s) chips.push(html`<span className="filter-chip" key="status">Status: ${s.name}<button onClick=${() => props.onFilter("statusId", "")}><${Icon} name="x" size=${13}/></button></span>`);
  }
  if (props.filters.bicId) {
    const a = actorById(data, props.filters.bicId); if (a) chips.push(html`<span className="filter-chip" key="bic">BIC: ${a.name}<button onClick=${() => props.onFilter("bicId", "")}><${Icon} name="x" size=${13}/></button></span>`);
  }
  if (props.filters.due !== "all") chips.push(html`<span className="filter-chip" key="due">Due: ${props.filters.due}<button onClick=${() => props.onFilter("due", "all")}><${Icon} name="x" size=${13}/></button></span>`);
  if (props.filters.priority) chips.push(html`<span className="filter-chip" key="priority">Priority: ${props.filters.priority}<button onClick=${() => props.onFilter("priority", "")}><${Icon} name="x" size=${13}/></button></span>`);
...
function TaskCard(props: any): any {
  const data: AppData = props.data;
  ...
  return html`<article className=${`task-card ${props.dragging ? "dragging" : ""}`} draggable=${true}
    onDragStart=${(e: any) => props.onDragStart(e, card.id)} onDragEnd=${props.onDragEnd} onClick=${() => props.onOpen(card.id)}>
    <div className="card-main">
      <div className="card-topline"><div className="card-title-wrap">${card.reference ? html`<div className="card-reference">${card.reference}</div>` : null}<div className="card-title">${card.title}</div>${data.settings.showDescriptions && card.description ? html`<div className="card-description">${card.description}</div>` : null}</div>
        ${items.length ? html`<button className="expand-button" onClick=${(e: any) => { stop(e); props.onToggleExpand(card.id); }} aria-label=${props.expanded ? "Collapse checklist" : "Expand checklist"}><${Icon} name=${props.expanded ? "chevronDown" : "chevronRight"} size=${16}/></button>` : null}
      </div>
...
function BoardView(props: any): any {
  ...
  const cardsFor = (col: any) => allCards.filter(c => axis === "topic" ? c.topicId === col.id : c.statusId === col.id).sort((a, b) => a.rank - b.rank);
  const renderColumnBody = (col: any): any => {
    const cards = cardsFor(col);
    if (!cards.length) return html`<div className="empty-column">Drop a task here or create a new one.</div>`;
    if (axis === "status") return cards.map(card => html`<${TaskCard} key=${card.id} data=${data} card=${card} expanded=${props.expanded.has(card.id)} dragging=${props.draggingId === card.id} onOpen=${props.onOpen} onToggleExpand=${props.onToggleExpand} onToggleItem=${props.onToggleItem} onDragStart=${props.onDragStart} onDragEnd=${props.onDragEnd}/>`);
    ...
    if (ungrouped.length) grouped.unshift({ id: "none", title: "General", cards: ungrouped });
  ...
  return html`<div className="view-contents">
    <${PageHeader} eyebrow=${props.selectedTopicId ? "Topic workspace" : "All active topics"} title=${props.selectedTopicId ? (topicById(data, props.selectedTopicId)?.name || "Board") : "Board"} subtitle=${axis === "topic" ? "Columns are Topics. Cards are grouped by Subtopic." : "Columns are active Statuses. Done tasks move to Archive."}>
    ...
    <div className="content-scroll flush"><div className="board-scroll"><div className="board">
      ${columns.map(col => html`<section key=${col.id} className=${`board-column ${props.dragOverId === col.id ? "drag-over" : ""}`} onDragOver=${(e: any) => props.onDragOver(e, col.id)} onDragLeave=${props.onDragLeave} onDrop=${(e: any) => props.onDrop(e, col.id, col.kind)}>
...
function ListView(props: any): any {
  const data: AppData = props.data;
  const cards: Card[] = props.cards.slice().sort((a, b) => {
    const da = nextDateFor(data, a) || "9999-99-99"; const db = nextDateFor(data, b) || "9999-99-99";
  ...
  return html`<div className="view-contents">
    ...
    <div className="content-scroll">
      ${cards.length ? html`<div className="data-table">
        ...
        ${cards.map(card => {
          const topic = topicById(data, card.topicId); const sub = subtopicById(data, card.subtopicId); const status = statusById(data, card.statusId); const bic = nextBicFor(data, card); const prog = progressFor(data, card); const items = checklistFor(data, card.id); const expanded = props.expanded.has(card.id);
          return html`<div key=${card.id}>
            <div className="table-row" onClick=${() => props.onOpen(card.id)}>
              <div className="table-task"><button className="expand-button" onClick=${(e: any) => { stop(e); props.onToggleExpand(card.id); }} disabled=${!items.length}><${Icon} name=${expanded ? "chevronDown" : "chevronRight"} size=${15}/></button><div className="card-title-wrap"><div className="table-title">${card.title}</div>${card.reference ? html`<div className="table-ref">${card.reference}</div>` : null}</div></div>
              <div className="table-cell-muted"><span className="topic-dot" style=${{ background: topic?.color || "#6b778c", display: "inline-block", marginRight: "6px" }}></span>${topic?.name || "—"}</div>
              <div className="table-cell-muted">${sub?.name || "—"}</div>
...
function FlowView(props: any): any {
  const data: AppData = props.data;
  const cards: Card[] = props.cards.slice().sort((a, b) => (nextDateFor(data, a) || "9999").localeCompare(nextDateFor(data, b) || "9999"));
  return html`<div className="view-contents">
...
function MiniTask(props: any): any {
  const data: AppData = props.data; const card: Card = props.card; const topic = topicById(data, card.topicId); const bic = nextBicFor(data, card);
  return html`<button className="mini-task" onClick=${() => props.onOpen(card.id)}><div><div className="mini-title">${card.reference ? `${card.reference} — ` : ""}${card.title}</div><div className="mini-meta"><span className="topic-dot" style=${{ background: topic?.color || "#6b778c", display: "inline-block", marginRight: "5px" }}></span>${topic?.name || "No topic"} · ${bic?.name || "Unassigned"}</div></div><${DueChip} date=${nextDateFor(data, card)}/></button>`;
}
function DashboardPanel(props: any): any {
  return html`<section className="panel"><div className="panel-header"><${Icon} name=${props.icon} size=${17}/><div className="panel-title">${props.title}</div><div className="panel-count">${props.cards.length}</div></div><div className="panel-body">${props.cards.length ? props.cards.slice(0, 6).map((card: Card) => html`<${MiniTask} key=${card.id} data=${props.data} card=${card} onOpen=${props.onOpen}/>`): html`<div className="panel-empty">Nothing here right now.</div>`}</div></section>`;
}
function DashboardView(props: any): any {
  const data: AppData = props.data; const cards: Card[] = props.cards;
  ...
  const metrics = [
    { label: "Due today", value: today.length, helper: "Needs attention", scope: "today" as SmartScope },
    { label: "Overdue", value: overdue.length, helper: "Open next actions", scope: "overdue" as SmartScope },
    { label: "My ball", value: mine.length, helper: "Assigned to me", scope: "my-ball" as SmartScope },
    { label: "Waiting on", value: waiting.length, helper: "Other parties", scope: "waiting" as SmartScope },
    { label: "No date", value: noDate.length, helper: "Needs scheduling", scope: "no-date" as SmartScope }
  ];
...
function ArchiveView(props: any): any {
  const data: AppData = props.data;
  ...
  return html`<div className="view-contents"><${PageHeader} eyebrow="Completed work and data safety" title="Archive & Backups" subtitle="Completed tasks are kept here and excluded from active workspaces.">
    ...
    <section className="panel"><div className="panel-header"><${Icon} name="archive" size=${17}/><div className="panel-title">Completed and archived tasks</div><div className="panel-count">${cards.length}</div></div><div className="panel-body" style=${{ padding: 0 }}>
      ${cards.length ? cards.map(card => { const topic = topicById(data, card.topicId); return html`<div className="archive-card" key=${card.id} role="button" tabIndex="0" onClick=${() => props.onOpen(card.id)} onKeyDown=${activateOnKey(() => props.onOpen(card.id))} title="Open this task"><div><div className="table-title">${card.reference ? `${card.reference} - ` : ""}${card.title}</div><div className="table-ref">${topic?.name || "No topic"} - ${card.archiveReason === "completed" ? "Completed" : "Archived"}</div></div><div className="table-cell-muted">${card.completedAt ? formatTimestamp(card.completedAt) : formatTimestamp(card.updatedAt)}</div><div><${StatusBadge} status=${statusById(data, card.statusId)}/></div><div className="archive-actions"><button className="btn btn-secondary btn-small" onClick=${(e: any) => { stop(e); props.onRestoreCard(card.id); }}><${Icon} name="undo" size=${14}/>Restore</button><button className="btn btn-danger btn-icon btn-small" title="Permanently delete" onClick=${(e: any) => { stop(e); props.onDeleteCard(card.id); }}><${Icon} name="trash" size=${14}/></button></div></div>`; }) : html`<div className="panel-empty">Completed tasks will be stored here.</div>`}
    </div></section>
    <div className="archive-side">
      <section className="panel"><div className="panel-header"><${Icon} name="hardDrive" size=${17}/><div className="panel-title">Backup center</div></div><div className="backup-card">
        <div className="setting-label">Local data location</div><div className="setting-help">${props.storageInfo?.mode || "Loading..."}</div><div className="storage-path">${props.storageInfo?.dataPath || "Loading local storage information..."}</div>
...
function SettingsView(props: any): any {
  const data: AppData = props.data;
  ...
  return html`<div className="view-contents"><${PageHeader} eyebrow="Workspace configuration" title="Settings" subtitle="Customize the default workspace layout, local startup behavior, and your hierarchy." />
    <div className="content-scroll"><div className="settings-layout">
      <section className="settings-card"><div className="settings-card-header"><div className="settings-card-title">Workspace</div><div className="settings-card-desc">Choose how the active Board is structured. Topic columns are the default.</div></div>
        ...
        <div className="setting-row"><div className="setting-copy"><div className="setting-label">Automatic backup retention</div><div className="setting-help">The desktop app keeps one automatic SQLite backup per active day. Manual and pre-restore backups remain until you delete them.</div></div><select className="select" style=${{ width: "180px" }} value=${String(data.settings.backupRetention)} onChange=${(e: any) => props.onSetting("backupRetention", Number(e.target.value))}><option value="7">7 automatic backups</option><option value="14">14 automatic backups</option><option value="30">30 automatic backups</option><option value="60">60 automatic backups</option><option value="90">90 automatic backups</option></select></div>
      </section>
...
class CreateTaskModal extends Component {
  ...
  setField = (name: string, value: any) => this.setState({ [name]: value });
  render(): any {
    const data: AppData = this.props.data;
    const subtopics = sortRank(data.subtopics.filter(s => !s.archived && s.topicId === this.state.topicId));
    return html`<div className="modal-backdrop" onMouseDown=${(e: any) => { if (e.target === e.currentTarget) this.props.onClose(); }}><form className="modal" onSubmit=${this.submit}>
      ...
      <div className="modal-body"><div className="form-grid">
        <div className="field full"><label>Task title *</label><input autoFocus className="input title-input" value=${this.state.title} onInput=${(e: any) => this.setField("title", e.target.value)} placeholder="What are you tracking?" /></div>
        <div className="field"><label>Reference</label><input className="input" value=${this.state.reference} onInput=${(e: any) => this.setField("reference", e.target.value)} placeholder="RFI-0102, PCO-0044…" /></div>
        <div className="field"><label>Card type</label><select className="select" value=${this.state.cardTypeId} onChange=${(e: any) => this.setField("cardTypeId", e.target.value)}>${sortRank(data.cardTypes.filter(t => !t.archived)).map(t => html`<option value=${t.id} key=${t.id}>${t.name}</option>`)}</select></div>
        <div className="field full"><label>Short description</label><textarea className="textarea" value=${this.state.description} onInput=${(e: any) => this.setField("description", e.target.value)} placeholder="A brief summary that is visible on the card." /></div>
        <div className="field"><label>Topic</label><select className="select" value=${this.state.topicId} onChange=${(e: any) => this.setState({ topicId: e.target.value, subtopicId: "" })}>${sortRank(data.topics.filter(t => !t.archived)).map(t => html`<option value=${t.id} key=${t.id}>${t.name}</option>`)}</select></div>
        <div className="field"><label>Subtopic</label><select className="select" value=${this.state.subtopicId} onChange=${(e: any) => this.setField("subtopicId", e.target.value)}><option value="">No subtopic</option>${subtopics.map(s => html`<option value=${s.id} key=${s.id}>${s.name}</option>`)}</select></div>
        <div className="field"><label>Status</label><select className="select" value=${this.state.statusId} onChange=${(e: any) => this.setField("statusId", e.target.value)}>${sortRank(data.statuses.filter(s => !s.terminal)).map(s => html`<option value=${s.id} key=${s.id}>${s.name}</option>`)}</select></div>
        <div className="field"><label>Priority</label><select className="select" value=${this.state.priority} onChange=${(e: any) => this.setField("priority", e.target.value)}>${["None", "Low", "Normal", "High", "Critical"].map(p => html`<option value=${p} key=${p}>${p}</option>`)}</select></div>
        <div className="field"><label>Overall target date</label><input className="input" type="date" value=${this.state.targetDate} onChange=${(e: any) => this.setField("targetDate", e.target.value)} /></div>
        <div className="field"><label>Fallback BIC</label><select className="select" value=${this.state.fallbackBicId} onChange=${(e: any) => this.setField("fallbackBicId", e.target.value)}><option value="">Unassigned</option>${sortRank(data.actors.filter(a => !a.archived)).map(a => html`<option value=${a.id} key=${a.id}>${a.name}</option>`)}</select></div>
        <div className="field full"><label>First checklist step (optional)</label><div className="inline-next"><input className="input" value=${this.state.nextTitle} onInput=${(e: any) => this.setField("nextTitle", e.target.value)} placeholder="Define the next action" /><input className="input" type="date" value=${this.state.nextDueDate} onChange=${(e: any) => this.setField("nextDueDate", e.target.value)} /><select className="select" value=${this.state.nextBicId} onChange=${(e: any) => this.setField("nextBicId", e.target.value)}><option value="">No BIC</option>${sortRank(data.actors.filter(a => !a.archived)).map(a => html`<option value=${a.id} key=${a.id}>${a.name}</option>`)}</select></div></div>
      </div></div>
...
function FilterPopover(props: any): any {
  const data: AppData = props.data; const f: Filters = props.filters;
  return html`<div className="filter-popover"><div className="filter-popover-title"><${Icon} name="filter" size=${17}/>Filter active tasks</div><div className="filter-grid">
    <div className="field"><label>Status</label><select className="select" value=${f.statusId} onChange=${(e: any) => props.onFilter("statusId", e.target.value)}><option value="">Any active status</option>${sortRank(data.statuses.filter(s => !s.terminal)).map(s => html`<option value=${s.id} key=${s.id}>${s.name}</option>`)}</select></div>
    <div className="field"><label>Ball in Court</label><select className="select" value=${f.bicId} onChange=${(e: any) => props.onFilter("bicId", e.target.value)}><option value="">Anybody</option>${sortRank(data.actors.filter(a => !a.archived)).map(a => html`<option value=${a.id} key=${a.id}>${a.name}</option>`)}</select></div>
...
function ImportWorkspaceModal(props: any): any {
  const candidate: ImportCandidate = props.candidate;
  ...
  return html`<div className="modal-backdrop"><div className="modal import-modal">
    ...
    <div className="modal-body">
      ...
      ${candidate.warnings.length ? html`<div className="import-warning"><${Icon} name="alert" size=${17}/><div>${candidate.warnings.map((warning: string) => html`<div key=${warning}>${warning}</div>`)}</div></div>` : null}
...
function checklistDepthMap(items: ChecklistItem[]): Map<string, number> {
  const map = new Map<string, number>();
  const byId = new Map(items.map(i => [i.id, i]));
  const depth = (item: ChecklistItem, seen: Set<string>): number => {
    if (!item.parentId || seen.has(item.id)) return 0;
    const parent = byId.get(item.parentId); if (!parent) return 0;
    seen.add(item.id); return Math.min(3, 1 + depth(parent, seen));
  };
  items.forEach(i => map.set(i.id, depth(i, new Set())));
  return map;
}

function CardDrawer(props: any): any {
  const data: AppData = props.data; const card: Card = props.card;
  const items = checklistFor(data, card.id); const depths = checklistDepthMap(items);
  const topicSubs = sortRank(data.subtopics.filter(s => !s.archived && s.topicId === card.topicId));
  const next = nextItemFor(data, card); const nextDate = nextDateFor(data, card); const bic = nextBicFor(data, card); const progress = progressFor(data, card);
  const update = (field: string, value: any) => props.onUpdateCard(card.id, field, value);
  return html`<div className="drawer-backdrop" onMouseDown=${(e: any) => { if (e.target === e.currentTarget) props.onClose(); }}><aside className="drawer">
    ...
    <div className="drawer-content">
      <div className="field"><label>Title</label><input className="input title-input" value=${card.title} onInput=${(e: any) => update("title", e.target.value)} /></div>
      <div className="form-grid" style=${{ marginTop: "12px" }}>
        <div className="field"><label>Reference</label><input className="input" value=${card.reference} onInput=${(e: any) => update("reference", e.target.value)} placeholder="Optional" /></div>
        <div className="field"><label>Status</label><select className="select" value=${card.statusId} onChange=${(e: any) => props.onStatus(card.id, e.target.value)}>${sortRank(data.statuses).map(s => html`<option value=${s.id} key=${s.id}>${s.name}</option>`)}</select></div>
        <div className="field full"><label>Short description</label><textarea className="textarea" value=${card.description} onInput=${(e: any) => update("description", e.target.value)} /></div>
        <div className="field"><label>Topic</label><select className="select" value=${card.topicId} onChange=${(e: any) => props.onMoveTopic(card.id, e.target.value)}>${sortRank(data.topics.filter(t => !t.archived)).map(t => html`<option value=${t.id} key=${t.id}>${t.name}</option>`)}</select></div>
        <div className="field"><label>Subtopic</label><select className="select" value=${card.subtopicId || ""} onChange=${(e: any) => update("subtopicId", e.target.value || null)}><option value="">No subtopic</option>${topicSubs.map(s => html`<option value=${s.id} key=${s.id}>${s.name}</option>`)}</select></div>
        <div className="field"><label>Card type</label><select className="select" value=${card.cardTypeId || ""} onChange=${(e: any) => update("cardTypeId", e.target.value || null)}><option value="">No type</option>${sortRank(data.cardTypes.filter(t => !t.archived)).map(t => html`<option value=${t.id} key=${t.id}>${t.name}</option>`)}</select></div>
        <div className="field"><label>Priority</label><select className="select" value=${card.priority} onChange=${(e: any) => update("priority", e.target.value)}>${["None", "Low", "Normal", "High", "Critical"].map(p => html`<option value=${p} key=${p}>${p}</option>`)}</select></div>
        <div className="field"><label>Overall target date</label><input className="input" type="date" value=${card.targetDate || ""} onChange=${(e: any) => update("targetDate", e.target.value || null)} /></div>
        <div className="field"><label>Fallback BIC</label><select className="select" value=${card.fallbackBicId || ""} onChange=${(e: any) => update("fallbackBicId", e.target.value || null)}><option value="">Unassigned</option>${sortRank(data.actors.filter(a => !a.archived)).map(a => html`<option value=${a.id} key=${a.id}>${a.name}</option>`)}</select></div>
      </div>
      <div className="section-title"><${Icon} name="sparkle" size=${15}/>Derived next action</div>
      <div className="card-next" style=${{ marginTop: 0 }}><div style=${{ display: "flex", alignItems: "center", gap: "9px" }}><div style=${{ flex: 1 }}><div className="card-next-label">Next action</div><div className="card-next-title" style=${{ fontSize: "13px" }}>${next?.title || "No open checklist item"}</div></div><${DueChip} date=${nextDate}/><${ActorPill} actor=${bic}/></div><div className="progress-wrap" style=${{ marginTop: "9px" }}><div className="progress-track"><div className="progress-fill" style=${{ width: `${progress.percent}%` }}></div></div><span className="progress-label">${progress.done} of ${progress.total} complete</span></div></div>
...
function RecoveryScreen(props: any): any {
  return html`<div className="recovery-page">
    <section className="recovery-card" role="alert">
      ...
      ${props.storageInfo?.dataPath ? html`<div className="recovery-detail"><strong>Database location</strong><code>${props.storageInfo.dataPath}</code></div>` : null}
...
interface RootState {
  ...
  filtersOpen: boolean;
  createOpen: boolean;
  createDefaults: { topicId?: string; statusId?: string; date?: string };
...
class RunningTaskApp extends Component {
  ...
  saveQueue = new SerializedSaveQueue<AppData>((data) => this.provider.save(data));
  saveTimer: any = null;
  toastTimer: any = null;
  closeUnlisten: null | (() => void) = null;
  ...
  componentWillUnmount(): void {
    ...
    if (this.closeUnlisten) this.closeUnlisten();
    if (this.saveTimer) clearTimeout(this.saveTimer);
    if (this.toastTimer) clearTimeout(this.toastTimer);
  }
  componentDidUpdate(_prevProps: any, prevState: RootState): void {
    if (this.state.data && prevState.data !== this.state.data) {
      if (this.suppressNextAutosave) this.suppressNextAutosave = false;
      else this.scheduleSave();
    }
    if (this.state.data && prevState.data?.settings.theme !== this.state.data.settings.theme) this.applyTheme(this.state.data.settings.theme);
    if (prevState.route !== this.state.route && this.state.route === "archive") this.refreshBackups();
  }
  loadWorkspace = async (): Promise<void> => {
    ...
    try {
      ...
      if (this.provider.desktop && data.settings.autostart) {
        try {
          ...
          if (!enabled) await this.provider.setAutostart(true);
        } catch (error) { console.warn("Autostart registration was not available.", error); }
      }
    } catch (error: any) {
      console.error(error);
      let storageInfo: StorageInfo | null = null;
      try { storageInfo = await this.provider.storageInfo(); } catch (_storageError) { /* keep recovery screen available */ }
      ...
      if (this.provider.desktop) {
        this.setState({ data: null, storageInfo, ready: true, loadError: error?.message || String(error), saveState: "error" });
      } else {
        const data = defaultData();
        this.applyTheme(data.settings.theme);
        this.setState({ data, storageInfo, ready: true, loadError: null, saveState: "saved", toast: { message: `Preview storage could not be opened: ${error?.message || error}`, type: "error" } });
      }
    }
  };
  /**
  ...
  setupDesktopCloseGuard = async (): Promise<void> => {
    ...
    if (!windowApi?.getCurrentWindow) return;
    try {
  ...
  onBeforeUnload = (event: BeforeUnloadEvent): void => {
    if (!this.saveQueue.hasWork()) return;
  ...
  onKeyDown = (event: KeyboardEvent): void => {
    if (event.key === "Escape") {
      ...
      else if (this.state.drawerCardId) this.setState({ drawerCardId: null });
      return;
    }
  ...
  flushPendingSave = async (silent = false): Promise<boolean> => {
    if (this.saveTimer) { clearTimeout(this.saveTimer); this.saveTimer = null; }
    if (!this.saveQueue.hasWork()) {
      if (this.mounted && this.state.saveState !== "saved") this.setState({ saveState: "saved" });
      return true;
    }
    if (this.mounted && this.state.saveState !== "saving") this.setState({ saveState: "saving" });
    try {
      await this.saveQueue.flush();
      if (this.mounted) this.setState({ saveState: "saved" });
      return true;
    } catch (error: any) {
      console.error(error);
      if (this.mounted) this.setState({ saveState: "error" });
      if (!silent) this.showToast(`Local save failed: ${error?.message || error}. Your latest changes remain queued; select Retry.`, "error");
      return false;
    }
  };
  ...
  setFilter = (key: keyof Filters, value: any): void => this.setState({ filters: Object.assign({}, this.state.filters, { [key]: value }) });
  clearFilters = (): void => this.setState({ filters: { statusId: "", bicId: "", due: "all", priority: "" }, scope: "", selectedTopicId: null });
  ...
  filteredCards(): Card[] {
    ...
    return data.cards.filter(card => !isDoneCard(data, card)).filter(card => {
      if (this.state.selectedTopicId && card.topicId !== this.state.selectedTopicId) return false;
      const date = nextDateFor(data, card); const offset = dayOffset(date); const bic = nextBicFor(data, card);
      ...
      if (f.bicId && bic?.id !== f.bicId) return false;
      if (f.priority && card.priority !== f.priority) return false;
      if (f.due === "overdue" && !(offset !== null && offset < 0)) return false;
      if (f.due === "today" && offset !== 0) return false;
      if (f.due === "7days" && !(offset !== null && offset >= 0 && offset <= 7)) return false;
      if (f.due === "30days" && !(offset !== null && offset >= 0 && offset <= 30)) return false;
      if (f.due === "nodate" && !!date) return false;
      if (q) {
        const topic = topicById(data, card.topicId); const sub = subtopicById(data, card.subtopicId); const type = typeById(data, card.cardTypeId);
        const itemText = checklistFor(data, card.id).map(i => `${i.title} ${actorById(data, i.bicId)?.name || ""}`).join(" ");
        const haystack = normalizeText(`${card.title} ${card.reference} ${card.description} ${card.notes} ${card.tags.join(" ")} ${topic?.name || ""} ${sub?.name || ""} ${type?.name || ""} ${bic?.name || ""} ${itemText}`);
  ...
  listTitle(): string {
    if (this.state.scope === "today") return "Due Today";
    if (this.state.scope === "overdue") return "Overdue";
    if (this.state.scope === "my-ball") return "My Ball";
    if (this.state.scope === "waiting") return "Waiting On Others";
    if (this.state.scope === "no-date") return "Tasks Without Dates";
    if (this.state.selectedTopicId && this.state.data) return topicById(this.state.data, this.state.selectedTopicId)?.name || "Task List";
    return "Task List";
  }
  openCreate = (defaults: any = {}): void => {
  ...
  createTask = (form: any): void => {
    const stamp = nowIso(); const cardId = uid("card");
    this.updateData(data => {
      ...
      if (form.nextTitle.trim()) data.checklistItems.push({ id: uid("item"), cardId, parentId: null, title: form.nextTitle.trim(), notes: "", dueDate: form.nextDueDate || null, bicId: form.nextBicId || null, completed: false, rank: 10, completedAt: null });
  ...
  setCardStatus = (cardId: string, statusId: string): void => {
    ...
    if (status?.terminal) { this.markDone(cardId); return; }
    this.updateData(draft => { const card = draft.cards.find(c => c.id === cardId); if (card) { card.statusId = statusId; card.updatedAt = nowIso(); } });
  };
  archiveCard = (cardId: string): void => {
    this.updateData(data => { const card = data.cards.find(c => c.id === cardId); if (card) { card.lastActiveStatusId = card.statusId; card.isArchived = true; card.archiveReason = "manual"; card.updatedAt = nowIso(); } }, "Task moved to Archive.");
    this.setState({ drawerCardId: null });
  };
  markDone = (cardId: string): void => {
  ...
  onDragEnd = (): void => this.setState({ draggingId: null, dragOverId: null });
  onDragOver = (event: any, id: string): void => { event.preventDefault(); event.dataTransfer.dropEffect = "move"; if (this.state.dragOverId !== id) this.setState({ dragOverId: id }); };
  onDrop = (event: any, columnId: string, kind: "topic" | "status"): void => {
    event.preventDefault(); const cardId = event.dataTransfer.getData("text/plain") || this.state.draggingId; if (!cardId) return;
  ...
  refreshBackups = async (): Promise<void> => {
    try { const [backups, storageInfo] = await Promise.all([this.provider.listBackups(), this.provider.storageInfo()]); this.setState({ backups, storageInfo }); }
    catch (error: any) { this.showToast(`Could not read backups: ${error?.message || error}`, "error"); }
  };
  createBackup = async (): Promise<void> => {
    if (!this.state.data || this.state.backupBusy) return;
    this.setState({ backupBusy: true });
    try {
      if (!await this.flushPendingSave(false)) throw new Error("The latest workspace changes could not be saved.");
      const backup = await this.provider.createBackup(this.state.data);
      await this.refreshBackups();
      this.showToast(`Backup created: ${backup.name}`);
    } catch (error: any) { this.showToast(`Backup failed: ${error?.message || error}`, "error"); }
  ...
  exportJson = async (): Promise<void> => {
    ...
    } catch (error: any) { this.showToast(`Export failed: ${error?.message || error}`, "error"); }
  };
  exportCsv = async (): Promise<void> => {
    ...
    } catch (error: any) { this.showToast(`CSV export failed: ${error?.message || error}`, "error"); }
  };
  exportMarkdown = async (): Promise<void> => {
    ...
    } catch (error: any) { this.showToast(`Markdown export failed: ${error?.message || error}`, "error"); }
  };
  prepareImportFile = async (file: File): Promise<void> => {
    this.setState({ importBusy: true });
    try {
      if (file.size > 50 * 1024 * 1024) throw new Error("The selected file is larger than the 50 MB import safety limit.");
      const raw = JSON.parse(await file.text());
      const prepared = prepareImportedWorkspace(raw);
      this.setState({ importCandidate: { fileName: file.name, ...prepared } });
    } catch (error: any) { this.showToast(`Import could not be prepared: ${error?.message || error}`, "error"); }
    finally { this.setState({ importBusy: false }); }
  };
  confirmImport = async (): Promise<void> => {
    if (!this.state.data || !this.state.importCandidate || this.state.importBusy) return;
    this.setState({ importBusy: true });
    try {
      if (!await this.flushPendingSave(false)) throw new Error("The latest workspace changes could not be saved before import.");
  ...
  renderMain(data: AppData, cards: Card[]): any {
    const common = {
      data, cards, selectedTopicId: this.state.selectedTopicId, scope: this.state.scope, filters: this.state.filters,
      onOpen: (id: string) => this.setState({ drawerCardId: id }), onCreate: () => this.openCreate(),
    ...
    if (this.state.route === "dashboard") return html`<${DashboardView} ...${common} cards=${data.cards.filter(c => !isDoneCard(data,c))} onNavigate=${this.navigate}/>`;
    if (this.state.route === "board") {
      const allExpanded = cards.length > 0 && cards.every(c => this.state.expanded.has(c.id));
      return html`<${BoardView} ...${common} expanded=${this.state.expanded} allExpanded=${allExpanded} onExpandAll=${() => this.expandAll(cards)} onAxis=${(axis: any) => this.setSetting("boardAxis", axis)} onAddTopic=${() => this.openEntityDialog("topic")} onQuickCreate=${(col: any) => this.openCreate(col.kind === "topic" ? { topicId: col.id } : { statusId: col.id })} draggingId=${this.state.draggingId} dragOverId=${this.state.dragOverId} onDragStart=${this.onDragStart} onDragEnd=${this.onDragEnd} onDragOver=${this.onDragOver} onDragLeave=${() => this.setState({ dragOverId: null })} onDrop=${this.onDrop}/>`;
    }
