...
Target branch for this release candidate: `release/v1.1.0`

## Current 1.1.0 status

The repository is initialized with build status, release documentation, and a Windows workflow. The complete RC source should be published to `release/v1.1.0`. The workflow is configured to build `release/**` branches, so the Windows release-candidate artifact can be tested before the branch is merged into `main`.
...
```text
Running_Task-1.1.0-github-web-upload.zip
```
...
9. Create or select branch `release/v1.1.0`, then commit the upload there. Merge to `main` only after the Windows acceptance gate passes.
...
- Use a tag such as `v1.1.0` only after the source commit and Windows Actions build succeed.
